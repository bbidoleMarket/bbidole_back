...
public class SalesListController {
    ...
    public ResponseEntity<ApiResponse<PageResDto<SalesListResDto>>> getLatestList(@PathVariable("id")Long userId, @RequestParam int page, @RequestParam int pageSize){//@AuthenticationPrincipal UserDetailsImpl userDetails
    ...
    public ResponseEntity<ApiResponse<PageResDto<SalesListResDto>>> getOnSalesList(@PathVariable("id")Long userId, @RequestParam int page, @RequestParam int pageSize){ //@AuthenticationPrincipal UserDetailsImpl userDetails
        //토큰에서 로그인한 사용자의 id 추출 추가 예정
        //Long userId = userDetails.getUser().getId();
    ...
    public ResponseEntity<ApiResponse<PageResDto<SalesListResDto>>> getCompletedSalesList(@PathVariable("id")Long userId, @RequestParam int page, @RequestParam int pageSize){ //@AuthenticationPrincipal UserDetailsImpl userDetails
        //토큰에서 로그인한 사용자의 id 추출 추가 예정
        //Long userId = userDetails.getUser().getId();
